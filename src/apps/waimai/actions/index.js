...
export const getNearStores = (channel, longitude, latitude, page) => dispatch => {
    request.get(`${window.$config.HOST}/waimai/v1/restaurants?channel=${channel}&longitude=${longitude}&latitude=${latitude}&offset=${page * 10}`).end((err, res) => {
        if (!err && res.statusCode === 200) {
            if (res.body.code !== 0) return alert(JSON.stringify(res.body.message))
            dispatch(getNearStoresReturn(res.body.data))
        }
    })
}

export const getNearStoresReturn = stores => {
    return {
        type: 'GET_NEAR_STORES', stores
    }
}

export const enterStore = (id, name, pay, invoice) => {
    return {
        type: 'ENTER_STORE', id, name, pay, invoice
    }
}
